  ...
  private async createLogGroup() {
    ...
    try {
      ...
      if (response.logGroups !== undefined && response.logGroups.length > 0) {
        const findGroup = response.logGroups.find((group) => {
          return group.logGroupName === this.logGroupName;
        });
        this.logGroup = findGroup ? findGroup : response.logGroups[0];
        return;
      }
  ...
  private async createLogStream() {
    ...
    try {
      ...
      if (response.logStreams) {
        const findStream = response.logStreams.find((stream) => {
          return stream.logStreamName === this.logStreamName;
        });
        this.logStream = findStream ? findStream : response.logStreams[0];
  ...
  async flush() {
    ...
    while (true) {
      ...
      try {
        ...
        if (error.name !== 'InvalidSequenceTokenException') throw error;
